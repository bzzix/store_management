...
<head>
    ...
    <style>
        ...
        body {
            width: 72mm; /* Further reduced to ensure no clipping */
            margin: 0 auto;
            padding: 1mm 0;
...
<body>
    ...
    <table>
        ...
        <tr>
            <td colspan="4" class="text-center font-bold" style="font-size: 13px;">فاتورة مبيعات رقم : {{ $invoice->invoice_number }}</td>
        </tr>
        <tr style="font-size: 9.5px;">
            <td class="label-bold" style="width: 13%;">العميل:</td>
            <td style="width: 37%;">{{ $invoice->customer?->name }}</td>
        ...
        <tr style="font-size: 9.5px;">
            ...
            <td class="label-bold">رقم السيارة:</td>
            <td>{{ $invoice->car_number ?? '---' }}</td>
        </tr>
        <tr style="font-size: 9.5px;">
            <td class="label-bold">العنوان:</td>
            <td>{{ $invoice->customer?->address ?? '---' }}</td>
            <td class="label-bold">السائق:</td>
    ...
    <table class="totals-grid">
        ...
        <tr>
            <td style="font-size: 9.5px;">كمية الوزن : <span class="font-bold">{{ number_format($invoice->items->sum('quantity_in_base_unit'), 2) }}</span></td>
            <td class="label-bold">رصيد سابق</td>
            <td class="text-center font-bold">{{ number_format($invoice->previous_balance, 2) }}</td>
        </tr>
        <tr>
            ...
            <td class="text-center font-bold">{{ number_format($invoice->subtotal + $invoice->previous_balance, 2) }}</td>
        </tr>
        <tr>
            <td>خصم</td>
        ...
        <tr>
            <td class="important-total">بعد الخصم</td>
            <td class="text-center important-total">{{ number_format(($invoice->subtotal + $invoice->previous_balance) - $invoice->discount_amount, 2) }}</td>
        </tr>
        <tr>
            <td class="label-bold">المدفوع</td>
            <td class="text-center font-bold">{{ number_format($invoice->paid_amount, 2) }}</td>
        </tr>
        <tr>
            <td class="important-total">المتبقي</td>
            <td class="text-center important-total">{{ number_format((($invoice->subtotal + $invoice->previous_balance) - $invoice->discount_amount) - $invoice->paid_amount, 2) }}</td>
        </tr>
    </table>
