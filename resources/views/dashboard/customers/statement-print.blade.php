...
<head>
    <meta charset="UTF-8">
    <title>{{ __('Customer Statement') }} - {{ $customer->display_name }}</title>
    <style>
        @page {
            size: 80mm auto;
            margin: 0;
...
<body>
    ...
    <table class="no-border">
        <tr>
            <td colspan="2" class="text-center font-bold" style="border:none; font-size: 11px;">{{ __('Customer Statement') }}</td>
        </tr>
        <tr>
            <td style="border:none; width: 25%;" class="font-bold">العميل:</td>
            <td style="border:none;">{{ $customer->display_name }}</td>
        </tr>
        <tr>
    ...
    <div class="balance-box text-center">
        الرصيد النهائي المستحق: {{ number_format($transactions['final_balance'], 2) }}
    </div>
