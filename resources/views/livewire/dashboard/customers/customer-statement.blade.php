<div class="space-y-8 max-w-5xl mx-auto">
    ...
    <div class="flex justify-between items-center no-print">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-bold text-surface-900 font-cairo leading-tight">{{ __('Customer Statement') }}</h2>
                <p class="text-surface-500 font-medium font-cairo">سجل الحركات المالية التفصيلي</p>
            </div>
        </div>
        <div class="flex gap-3">
            <a href="{{ route('dashboard.customers.index') }}" class="px-6 py-2.5 rounded-2xl bg-white border border-surface-200 text-surface-700 font-bold font-cairo hover:bg-surface-50 transition-all shadow-sm">
                {{ __('Back') }}
            </a>
            <a href="{{ route('dashboard.customers.statement.print', ['customer' => $customer->id, 'fromDate' => $fromDate, 'toDate' => $toDate]) }}" 
                target="_blank"
                class="px-6 py-2.5 rounded-2xl bg-primary-600 text-white font-bold font-cairo hover:bg-primary-700 transition-all shadow-lg shadow-primary-200 flex items-center gap-2">
    ...
    <div class="bg-white border-2 border-surface-200 rounded-[2rem] overflow-hidden shadow-xl print:shadow-none print:border-none">
        ...
        <div class="p-8 border-b border-surface-100 bg-surface-50/30">
            <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                <div class="text-center md:text-right">
                    ...
                    @else
                        <h1 class="text-3xl font-bold text-surface-900 font-cairo">{{ get_setting('appName', __('ABDELSTAR AGRI')) }}</h1>
                    @endif
                    <p class="text-surface-600 font-bold mt-1 font-cairo text-lg">{{ __('Management') }} / {{ get_setting('appManagerName', 'محمود حسن') }}</p>
                </div>
                
                <div class="text-center">
                    <div class="inline-block px-6 py-2 rounded-2xl bg-primary-600 text-white font-bold text-xl font-cairo mb-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-surface-100 border-b border-surface-100">
            ...
            <div class="p-8 space-y-4">
                <h3 class="text-sm font-bold text-primary-600 uppercase tracking-wider font-cairo">{{ __('Customer Information') }}</h3>
                <div class="space-y-2">
                    <div class="flex items-center gap-3">
                        <span class="text-surface-400 text-sm font-cairo">{{ __('Name') }}:</span>
                        <span class="text-surface-900 font-bold font-cairo">{{ $customer->display_name }}</span>
                    </div>
                    <div class="flex items-center gap-3">
            ...
            <div class="p-8 space-y-6">
                <h3 class="text-sm font-bold text-primary-600 uppercase tracking-wider font-cairo">{{ __('Filtering & Period') }}</h3>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-surface-500 font-cairo">{{ __('From Date') }}</label>
                    ...
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-surface-500 font-cairo">{{ __('To Date') }}</label>
        ...
        <div class="p-0 overflow-x-auto">
            <table class="w-full text-right border-collapse">
                <thead>
                    <tr class="bg-surface-50/50 border-b-2 border-surface-100 uppercase tracking-wider text-xs font-bold text-surface-600 font-cairo">
                ...
                <tbody class="divide-y divide-surface-100">
                    ...
                    <tr class="bg-primary-50/10 font-medium">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="text-xs font-bold text-primary-600 font-cairo block leading-none mb-1">{{ __('Opening Balance') }}</span>
                        ...
                        @foreach ($items as $index => $item)
                            <tr class="hover:bg-surface-50 transition-all group">
                                ...
                                <td class="px-4 py-4 text-sm text-surface-900 font-outfit text-center font-medium">
                                    {{ $item['type'] === 'invoice' ? number_format($item['value'], 2) : '-' }}
                                </td>
        ...
        {{-- Totals Grid - Matching Invoice Style --}}
            ...
            <div class="w-full md:w-96 space-y-3">
                <div class="grid grid-cols-2 gap-3">
                    <div class="flex flex-col p-4 rounded-3xl bg-white border border-surface-100 shadow-sm">
                        <span class="text-xs font-bold text-surface-500 font-cairo mb-1">{{ __('Previous Balance') }}</span>
                        <span class="text-xl font-bold text-surface-900 font-outfit">{{ number_format($transactions['previous_balance'], 2) }}</span>
                    </div>
                    <div class="flex flex-col p-4 rounded-3xl bg-white border border-surface-100 shadow-sm">
                        <span class="text-xs font-bold text-primary-600 font-cairo mb-1">عدد الحركات</span>
                ...
                <div class="flex justify-between items-center py-3 px-5 rounded-2xl bg-white border border-surface-100">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-bold text-surface-700 font-cairo">{{ __('Total Addition') }}</span>
                    </div>
                ...
                <div class="flex justify-between items-center py-3 px-5 rounded-2xl bg-white border border-surface-100">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-bold text-surface-700 font-cairo">{{ __('Total Deduction') }}</span>
                    </div>
                ...
                <div class="flex justify-between items-center p-6 rounded-[2rem] bg-surface-900 text-white shadow-2xl shadow-surface-200">
                    <span class="text-lg font-bold font-cairo">{{ __('Final Balance') }}</span>
                    <div class="text-right">
                        <span class="text-3xl font-bold font-outfit block">{{ number_format($transactions['final_balance'], 2) }}</span>
                        <span class="text-[10px] opacity-70 font-cairo">{{ $transactions['final_balance'] > 0 ? 'رصيد مستحق للشركة' : 'رصيد دائن للعميل' }}</span>
                    </div>
                </div>
    ...
    <div wire:loading class="fixed inset-0 bg-surface-900/10 backdrop-blur-sm z-50 flex items-center justify-center">
        <div class="bg-white p-6 rounded-3xl shadow-2xl flex flex-col items-center gap-4">
            ...
            <span class="text-surface-600 font-bold font-cairo">{{ __('Processing...') }}</span>
        </div>
    </div>
</div>
