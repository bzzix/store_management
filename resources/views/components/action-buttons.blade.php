<div class="flex items-center gap-1.5" role="group">
    {{-- Edit Button --}}
    <button type="button" 
            wire:click="editTier({{ $row->id }})"
            wire:loading.attr="disabled"
            wire:target="editTier({{ $row->id }})"
            class="p-1.5 rounded-lg bg-primary-50 text-primary-600 hover:bg-primary-100 transition-colors"
            title="{{ __('Edit') }}">
        <span wire:loading.remove wire:target="editTier({{ $row->id }})">
        ...
        <span wire:loading wire:target="editTier({{ $row->id }})">
